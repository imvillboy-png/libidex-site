...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name) || empty($review_text)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO reviews (name, age, image, review_text, sort_order, status) VALUES (?, ?, ?, ?, ?, ?)");
        
        try {
            $stmt->execute([$name, $age, $imagePath, $review_text, $sort_order, $status]);
...
<head>
    ...
    <title>Add Review - Libidex Admin</title>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="age" name="age" class="form-control" placeholder="e.g., 35 वर्ष">
                            </div>
                                    ...
                                    <div style="display: flex; gap: 12px; flex-wrap: wrap;">
                                        <?php foreach ($existingImages as $img): ?>
                                            <label style="cursor: pointer;">
                                                <input type="radio" name="existing_image" value="<?php echo $img; ?>" style="display: none;">
                                                <img src="../images/<?php echo $img; ?>" style="width: 80px; height: 80px; object-fit: cover; border-radius: 8px; border: 2px solid transparent;">
                                            </label>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                        ...
                        <div class="btn-group">
                            <button type="submit" class="btn btn-add">Create Review</button>
